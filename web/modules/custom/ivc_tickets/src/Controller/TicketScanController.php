...
class TicketScanController extends ControllerBase {
  ...
  public function ticket_scan($hash) {
    ...
    if ($tids) {
      ...
      */
      ...
      if ($valid && $ticket->get('field_redeemed')->value) {
      ...
      if ($valid && $ticket->get('field_admission_date_and_time')->value && strlen($ticket->get('field_admission_date_and_time')->value) > 0) {
        ...
        if (( $now < $date_start ) && (abs($date_start - $now) > 36000)) {
        ...
        } else if ($today != date('m/d/Y', $date_start)) {
    ...
    $ret[] = [
      'scan_another' => [
        '#markup' => '<h2><a class="btn-scan" href="javascript:void(0)">Scan Another Ticket</a></h2>',
      ],
  ...
  public function scanner() {
    ...
    return [
      'scan' => [
        '#markup' => '<h2><a class="btn-scan" href="javascript:void(0)">Scan a Ticket</a></h2>',
      ],      
      'ticket_scanner_block' => $render,
    ];
  }
